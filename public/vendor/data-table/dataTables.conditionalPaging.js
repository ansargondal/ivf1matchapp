            ...
            var config = $.isPlainObject(options) ? options : {},
                ...
                conditionalPaging = function (e) {
                    var $paging = $(api.table().container()).find('div.dataTables_paginate'),
                        $dt_info = $(api.table().container()).find('div.dataTables_info'),
                        $table = $(api.table().container()).find('table'),
                    ...
                    if (e instanceof $.Event) {
                        if (pages <= 1) {
                            if (config.style === 'fade') {
                                ...
                                $dt_info.css('visibility', 'hidden');
                            }
                        }
                        else {
                            $table.css('padding-bottom', '90px');
